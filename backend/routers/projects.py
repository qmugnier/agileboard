...
from database import get_db, Project, ProjectStatus, StatusTransition, Epic, Sprint, UserStory, TeamMember, DailyUpdate
from schemas import (
...
def create_project(project: ProjectCreate, db: Session = Depends(get_db)):
    ...
    default_statuses = [
        ...
        ProjectStatus(project_id=db_project.id, status_name="done", color="#10B981", order=3, is_locked=1, is_final=0, is_default=1),
    ]
...
def delete_project(project_id: int, db: Session = Depends(get_db)):
    """Delete project (only if no attached user stories)"""
    project = db.query(Project).filter(Project.id == project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    
    story_count = db.query(UserStory).filter(UserStory.project_id == project_id).count()
    if story_count > 0:
        raise HTTPException(status_code=400, detail=f"Cannot delete project: {story_count} user stories attached")
